...
int main() {
    ...
    for (const auto& b : blocks) {
        ...
        // Expected MONAD block pattern; pull the 3 constants.
        //  4: div z A
        //  5: add x B
        // 15: add y C  (the constant after "add y w")
        Block blk;
        blk.div_z = last_int(b[4]);
        blk.add_x = last_int(b[5]);
        blk.add_y = last_int(b[15]);
        params.push_back(blk);
    }

    // Build constraints using the standard stack interpretation:
    // If div_z==1: push (i, add_y)
    // If div_z==26: pop (j, add_y_j) => digit[i] = digit[j] + add_y_j + add_x_i
    ...
    // Maximize the 14-digit number lexicographically (leftmost digit prioritized).
