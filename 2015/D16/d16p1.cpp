...
int main() {
    ...
    while (std::getline(std::cin, line)) {
        ...
        while (std::getline(prop_stream, property, ',')) {
            ...
            // Check if the property matches the MFCSAM output
            auto it = mfcsam.find(key);
            if (it == mfcsam.end() || it->second != value) {
                match = false;
                break; // Property does not match, no need to check further
            }
        }

        // If all properties match, output the Sue number and terminate
        if (match) {
