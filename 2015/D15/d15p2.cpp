...
int main() {
    ...
    // Variable to keep track of the maximum score with exactly 500 calories
    ...
    std::function<void(size_t, int)> backtrack = [&](size_t index, int remaining) {
        if (index == num_ingredients - 1) {
            ...
            // Compute the total calories for the current distribution
            int total_calories = 0;
            for (size_t i = 0; i < num_ingredients; ++i) {
                total_calories += ingredients[i].calories * amounts[i];
            }

            // Only consider this distribution if total calories is exactly 500
    ...
    // Output the maximum score with exactly 500 calories
