...
int main() {
    ...
    constexpr std::size_t kConnect = 1000;
    const std::size_t max_pairs = static_cast<std::size_t>(n) * static_cast<std::size_t>(n - 1) / 2;
    const std::size_t target = std::min(kConnect, max_pairs);
    ...
    std::priority_queue<PairInfo, std::vector<PairInfo>, HeapCmp> pq;
    for (int i = 0; i < n; ++i) {
        for (int j = i + 1; j < n; ++j) {
    ...
    long long answer = 1;
    int take = std::min(3, static_cast<int>(comp_sizes.size()));
