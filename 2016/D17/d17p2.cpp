...
// Computes MD5 hash of input and returns its lowercase hex string.
class MD5 {
public:
    MD5() { init(); }
...
// Represents a position in the maze and the path taken so far.
struct Node {
    int x, y;
    std::string path;
};
...
int main() {
    ...
    const int targetX = WIDTH - 1, targetY = HEIGHT - 1;

    // Moves: U, D, L, R with corresponding delta x,y
    const std::array<char,4> dirs = {'U','D','L','R'};
    ...
    while (!stack.empty()) {
        ...
        // Compute hash of passcode + current path
        std::string h = MD5::hash(passcode + cur.path);

        // Try each direction in U,D,L,R order
        for (int i = 0; i < 4; ++i) {
            char c = h[i];
            bool open = (c >= 'b' && c <= 'f');
            int nx = cur.x + dx[i], ny = cur.y + dy[i];
            if (!open || nx < 0 || nx >= WIDTH || ny < 0 || ny >= HEIGHT) {
                continue;
            }
            // If this move reaches the vault, update longest length
            if (nx == targetX && ny == targetY) {
                int len = static_cast<int>(cur.path.size()) + 1;
                longest = std::max(longest, len);
            } else {
                // Otherwise continue exploring from the new position
                stack.push_back({nx, ny, cur.path + dirs[i]});
            }
        }
    }
