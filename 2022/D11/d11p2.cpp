...
int main() {
    ...
    // Compute the Least Common Multiple (LCM) of all test_divisors to manage worry levels
    long lcm = 1;
    for (const auto& monkey : monkeys) {
        lcm = std::lcm(lcm, static_cast<long>(monkey.test_divisor));
    }
    ...
    for (int round = 0; round < TOTAL_ROUNDS; ++round) {
        for (size_t i = 0; i < monkeys.size(); ++i) {
            ...
            while (!monkey.items.empty()) {
                ...
                // Manage worry levels by taking modulo with LCM to prevent overflow
                new_worry %= lcm;
    ...
    inspection_counts.reserve(monkeys.size());
    for (const auto& monkey : monkeys) {
        inspection_counts.push_back(monkey.inspect_count);
    }

    // Find the two highest inspection counts
    std::nth_element(inspection_counts.begin(), inspection_counts.begin() + 1, inspection_counts.end(), std::greater<long>());
    std::nth_element(inspection_counts.begin(), inspection_counts.begin(), inspection_counts.end(), std::greater<long>());
