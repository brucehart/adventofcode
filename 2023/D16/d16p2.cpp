...
int main() {
    ...
    // Top row
    for (int j = 0; j < cols; ++j) {
        max_energized = std::max(max_energized, simulate({-1, j, 1}));
    }

    // Bottom row
    for (int j = 0; j < cols; ++j) {
        max_energized = std::max(max_energized, simulate({rows, j, 3}));
    }

    // Left column
    for (int i = 0; i < rows; ++i) {
        max_energized = std::max(max_energized, simulate({i, -1, 0}));
    }

    // Right column
    for (int i = 0; i < rows; ++i) {
        max_energized = std::max(max_energized, simulate({i, cols, 2}));
    }
