...
int main() {
    ...
    std::vector<std::vector<std::vector<std::vector<bool>>>> visited(
        rows, std::vector<std::vector<std::vector<bool>>>(
            cols, std::vector<std::vector<bool>>(
                4, std::vector<bool>(11, false) // Max steps is now 10
            )
        )
    );
    ...
    while (!pq.empty()) {
        ...
        if (current.row == rows - 1 && current.col == cols - 1 && current.steps >=4) {
        ...
        for (int newDir = 0; newDir < 4; ++newDir) {
            ...
            // Must move at least 4 steps before turning
            if (newDir != current.dir && current.steps < 4 && (current.row !=0 || current.col !=0))
            {
                continue;
            }
            ...
            // Can't move more than 10 steps in the same direction
            if (newSteps > 10) {
                continue;
            }
